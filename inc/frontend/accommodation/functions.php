...
if ( ! function_exists( 'trav_acc_get_search_result' ) ) {
	function trav_acc_get_search_result( $s='', $date_from='', $date_to='', $rooms=1, $adults=1, $kids=0, $order_by='acc_title', $order='ASC', $last_no=0, $per_page=12, $min_price=0, $max_price='no_max', $rating, $acc_type, $amenities ) {
		...
		if ( ! empty( $rating ) && is_array( $rating ) ) {
			$rating_sql = array();
			foreach ( $rating as $r ) {
				$rating_sql[] = "( IFNULL( cast(meta_rating.meta_value as DECIMAL(2,0)), 0 ) = {$r} )";
			}
			$where .= " AND ( " . implode( 'OR', $rating_sql ) . " )";
		...
		foreach ( $booking_fields as $field ) {
			if ( ! isset( $raw_booking_data[ $field ] ) ) {
				if ( $field == 'child_ages' ) {
					...
					$booking_data['child_ages'] = serialize( $child_ages );
