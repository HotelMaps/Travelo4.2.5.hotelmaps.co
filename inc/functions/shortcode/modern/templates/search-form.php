...
function trav_shortcode_search_form( $atts, $content = null ) {
...
add_shortcode( 'trav_modern_search_form', 'trav_shortcode_search_form' );

/**
 * WPBakery
 */
function trav_vc_shortcode_search_form() {

	$extra_class = trav_extra_class_field();

	vc_map( array(
		'name'			=>	esc_html__( 'Search Form', 'trav' ),
		'base'			=>	'trav_modern_search_form',
		'icon'			=>	'trav-js-composer',
		'category'		=>	esc_html__( 'by C-Themes', 'trav' ),
		'description'	=>	esc_html__( 'Show search form in your page.', 'trav' ),
		'params'		=>	array(
			array(
				"type"			=> "dropdown",
				"class"			=> "",
				"heading"		=> esc_html__( "Post Type", 'trav' ),
				"param_name"	=> "post_type",
				'value' => array(
					esc_html__( 'Accommodation', 'trav' )	=> 'accommodation',
					esc_html__( 'Tour', 'trav' )			=> 'tour',
					esc_html__( 'Post', 'trav' )			=> 'post',
				),
				"std"			=> "post",
			),
			$extra_class,
